...
class SecurityTrailsClient:
    def __init__(self, api_key: Optional[str] = None, base_url: str = DEFAULT_BASE, timeout: int = 15):
        ...
        self.session.headers.update({
            "Accept": "application/json",
            "APIKEY": self.api_key,
            "User-Agent": "BlackByt3-Subenum/1.0 (by Jahangir)"
        })
        self.timeout = timeout
...
# Utility
# ========================================================
def save_to_file(hosts: List[str], path: str = "extracted_subdomains.txt"):
    with open(path, "w") as f:
        for h in hosts:
            f.write(h + "\n")
...
# ========================================================
if __name__ == "__main__":
    print("===== SecurityTrails Subdomain Extractor =====")
    
    domain = input("[1] Enter target domain (e.g. example.com): ").strip()
    api_key = input("[2] Enter your SecurityTrails API key: ").strip()
    
    print("\n[3] Choose method:")
    print("   1. list   (simple subdomain listing)")
    print("   2. search (advanced search, paginated)")
    method_choice = input("Enter choice (1 or 2): ").strip()
    method = "list" if method_choice == "1" else "search"
    ...
    if method == "list":
        ...
        subs, raw = client.search_subdomains(domain)

    # Print results
    print(f"\nFound {len(subs)} subdomains for {domain} via SecurityTrails ({method})")
    for s in subs:
        print(s)

    # Always save results
    save_to_file(subs)
    print("\n[INFO] Results saved to extracted_subdomains.txt")
